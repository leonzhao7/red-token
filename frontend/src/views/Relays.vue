...
function setConsoleLogRows(rows: ConsoleRequestLogRow[]) {
  consoleLogRows.value = rows
  expandedLogRowIds.value = new Set()
}

function toggleLogRow(id: string) {
  const next = new Set(expandedLogRowIds.value)
  next.has(id) ? next.delete(id) : next.add(id)
  expandedLogRowIds.value = next
}

function clearConsoleLogRows() {
  consoleLogRows.value = []
  expandedLogRowIds.value = new Set()
}
...
function formatLogBody(body: string) {
  try { return JSON.stringify(JSON.parse(body), null, 2) } catch { return body }
}

function formatLogPreview(body: string) {
  const text = body ? body.replace(/\s+/g, ' ').trim() : ''
  return text.length <= 140 ? text : `${text.slice(0, 140)}...`
}

function formatLogStatus(statusCode: number | null) {
  return statusCode && statusCode > 0 ? String(statusCode) : 'No response'
}

function logStatusClass(statusCode: number | null) {
...
function appendLogRow(req: { time: string; method?: string; path: string; status_code: number; body: string }, backendName: string) {
...
<template>
  <div class="page stagger">
    ...
    <!-- console request log -->
    <Modal
      :open="showConsoleLogModal"
      title="控制台请求日志"
      :subtitle="consoleLogTitle"
      width="960px"
      @close="showConsoleLogModal = false"
    >
...
.console-log-context { display: flex; align-items: center; gap: 10px; }
.console-log-context strong { font-size: 13px; font-weight: 600; color: var(--text); }
.console-log-context span { font-size: 12px; color: var(--text-faint); }
