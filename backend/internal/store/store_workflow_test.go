...
func TestApplyHTTPWorkflowResultUpdatesBackendAndSnapshotAtomically(t *testing.T) {
	...
	updated.ConsoleRefreshToken = "refresh-token-new"
	...
	if gotBackend.APIKey != "new-key" || gotBackend.ConsoleAccountJSON != updated.ConsoleAccountJSON || gotBackend.ConsolePricingJSON != updated.ConsolePricingJSON || gotBackend.ConsoleHeaders["Cookie"] != "session=new" || gotBackend.ConsoleHeaders["X-Test"] != "configured" || gotBackend.ConsoleRefreshToken != "refresh-token-new" {
	...
	rollbackCandidate.ConsoleRefreshToken = "refresh-token-should-rollback"
	...
	if afterRollback.APIKey != gotBackend.APIKey || afterRollback.ConsoleAccountJSON != gotBackend.ConsoleAccountJSON || afterRollback.ConsoleHeaders["Cookie"] != gotBackend.ConsoleHeaders["Cookie"] || afterRollback.ConsoleRefreshToken != gotBackend.ConsoleRefreshToken {
